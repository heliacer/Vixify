...
@Guard(IsGuildUser)
@SlashGroup({ name: "currency", description: "Currency settings", root: "config" })
...
export class ConfigCurrency {
    ...
    name(
        ...
        if (!interaction.guildId) return interaction.reply({ content: config.commandOnlyInGuildWarning, flags: ["Ephemeral"] })
        getOrCreateGuild({ id: interaction.guildId, currencyName: singular, currencyNamePlural: plural })
        return interaction.reply(`Currency name set to:\nSingular: **${singular}**\nPlural: **${plural}**`)
    }
