...
export class Wallet {
    ...
    async wallet(
        ...
        const walletContent = sortedRecords.map((record) => {
            const emoji = record.guild.currencyEmoji || emojis.gelt
            const coins = record.currency
            const guildName =
                interaction.client.guilds.cache.get(record.guildId)?.name || "Server"
        ...
        const embed = new EmbedBuilder()
            .setAuthor({ name: discordUser.displayName, iconURL: discordUser.displayAvatarURL() })
